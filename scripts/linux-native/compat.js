...
const BUILD_INPUT_FILES = Object.freeze(['Dockerfile', 'build.js', 'compat.js', 'container-build.js'])
...
function inspectNativeAddon(addonPath, versionSource) {
  const buffer = fs.readFileSync(addonPath)
  const requirements = parseVersionRequirements(versionSource ?? buffer.toString('latin1'))
...
function createNativeManifest({ addonPath, metadata, arch, toolchain, versionSource }) {
  const inspection = inspectNativeAddon(addonPath, versionSource)
  if (inspection.arch !== arch) {
    throw new Error(`Expected a ${arch} better-sqlite3 artifact, found ${inspection.arch}`)
  }
  ...
  return {
    ...
    electronAbi: metadata.electronAbi,
    betterSqlite3Version: metadata.betterSqlite3Version,
    electronRebuildVersion: metadata.electronRebuildVersion,
...
function getNativeArtifactPaths(projectRoot, arch) {
  const outputDir = path.join(projectRoot, 'scripts', 'linux-native', 'prebuilt', arch)
  return {
    outputDir,
...
function verifyNativeArtifact({ addonPath, manifestPath, expected }) {
  ...
  if (JSON.stringify(manifest.limits) !== JSON.stringify(VERSION_LIMITS)) {
    throw new Error('better-sqlite3 manifest compatibility limits are stale')
  }
...
function computeBuildFingerprint(projectRoot, metadata) {
  const hash = crypto.createHash('sha256')
  for (const file of BUILD_INPUT_FILES) {
    hash.update(file)
    hash.update('\0')
    hash.update(fs.readFileSync(path.join(projectRoot, 'scripts', 'linux-native', file)))
    hash.update('\0')
  }
...
function readProjectBuildMetadata(projectRoot) {
  ...
  const metadata = {
    ...
    electronRebuildVersion: readInstalledPackageVersion(projectRoot, '@electron/rebuild')
  }
  return {
    ...metadata,
    buildFingerprint: computeBuildFingerprint(projectRoot, metadata)
  }
}
